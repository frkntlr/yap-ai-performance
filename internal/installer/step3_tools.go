...
func Step3Tools(p *detector.Platform, ctx *context.RunContext) error {
	...
	if err == nil && strings.Contains(pipxList, "codegraphcontext") {
		...
		_ = runner.Run(ctx.DryRun, "pipx", "upgrade", "codegraphcontext")
	} else {
		if err := runner.Run(ctx.DryRun, "pipx", "install", "codegraphcontext"); err != nil {
	...
	if err := runner.Run(ctx.DryRun, "uv", "tool", "install", "--force", "graphifyy[mcp]"); err != nil {
		...
		if err := runner.Run(ctx.DryRun, pythonCmd, "-m", "pip", "install", "--user", "graphifyy[mcp]"); err != nil {
